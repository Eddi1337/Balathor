...
function planetReturnPortal(planet) {
  return {
    id: `portal_${planet.id}_return`,
    name: `${planet.name} Orbit`,
    // Surface center +/- the same offset as the PORTAL tile (north of spawn).
    x: planet.surfaceX,
    y: planet.surfaceY - 3,
    targetX: planet.x,
    targetY: planet.y,
...
function getPortalAtTile(x, y) {
  const direct = PORTALS.find((portal) => portal.x === x && portal.y === y);
  if (direct) return direct;
  // Planet return portals are exposed as PORTAL tiles in the surface generator.
  for (const planet of SCI_FI_PLANETS) {
    const px = Math.round(planet.surfaceX);
    const py = Math.round(planet.surfaceY - 3);
    if (px === x && py === y) return planetReturnPortal(planet);
  }
  return null;
}
...
function getPortalAt(x, y) {
  ...
  for (const planet of SCI_FI_PLANETS) {
    const px = planet.surfaceX;
    const py = planet.surfaceY - 3;
    if (Math.hypot(x - px, y - py) <= PORTAL_RADIUS) {
      return planetReturnPortal(planet);
    }
  }
  return null;
}
...
function generateExteriorTile(x, y) {
  ...
  // Procedural alien planet surfaces live at remote coordinates and short-circuit
  // any other tile generation when the point falls inside their disc.
  const surfacePlanet = getPlanetBySurfacePoint(x, y);
  if (surfacePlanet) {
